...
public sealed class MentorshipRequestRepository : IMentorshipRequestRepository
{
    ...
    public async Task<IReadOnlyList<SentMentorshipRequestReadModel>> GetSentByMentorId(Guid learnerId)
    {
        return await(
            from request in _dbContext.MentorshipRequests.AsNoTracking()
            join mentor in _dbContext.Mentors.AsNoTracking()
                on request.MentorId equals mentor.Id
            join stack in _dbContext.Stacks.AsNoTracking()
                on request.StackId equals stack.Id
            where request.LearnerId == learnerId
            orderby request.CreatedAt descending
            select new SentMentorshipRequestReadModel(
                request.Id,
                mentor.Id,
